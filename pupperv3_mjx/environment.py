...
from pupperv3_mjx.obstacles import add_stairs_to_model
...
class PupperV3Env(PipelineEnv):
    ...
    def __init__(
        ...
        """
        # Load and modify the MJCF model to add stairs
        '''
        tree = ET.parse(path)
        add_stairs_to_model(
            tree,
        ...
        updated_path="pupper_v3_description/description/mujoco_xml/model_with_obstacles.xml"
        tree.write(updated_path)
        '''
        # Use the modified MJCF model for the environment
        sys = mjcf.load(path)
